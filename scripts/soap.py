...
# Default cusolver eigh has been observed to crash (illegal memory access,
# which corrupts the CUDA context for the rest of the process) specifically
# on ~2048x2048+ preconditioner matrices, in this project's pytorch
# 2.2.0/cuda 11.8 image. Only relevant if max_precond_dim is set large enough
# for axes that size to actually be preconditioned (see train.py's
# --soap-max-precond-dim / --soap-linalg-backend) -- caller's responsibility
# to pick "magma" via torch.backends.cuda.preferred_linalg_library() in that
# case; below that size cusolver ("default") is both safe and faster.
